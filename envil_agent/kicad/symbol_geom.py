...
def _discover_config_roots() -> Tuple[str, ...]:
    ...
    for cfg_dir in _kicad_config_dirs():
        # Seed with the synthesized KICAD<major>_SYMBOL_DIR BEFORE reading
        # kicad_common.json, so an explicit user-set var (rare) still wins —
        # dict.update below only fills in keys kicad_common.json didn't set.
        ...
        try:
            if common.is_file():
                ...
                json_env = raw_env.get("vars", raw_env) if isinstance(raw_env, dict) else {}
